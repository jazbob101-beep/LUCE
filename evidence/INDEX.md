...
| V004 RUN-2 immutable black-box archive | **Read-only raw specimen** — `/mnt/d/Codex/TT3/bench-evidence/v004-run2-20260903/raw/RUN-2-blackbox.tar.gz`; 9,434 bytes, SHA-256 `dae20f7af9f8c08526a7f715182d7f87268b9e88de760fc48d846ede5315722f`; adjacent `ACQUISITION.md` and `RAW_MANIFEST.json`; 25 regular files/204,359 bytes. | Fourteen cumulative dmesg snapshots retain history through uptime 311.35 s; 229 polls continue through 316.02 s, including a 4.67 s poll-only tail. No physical event timestamp or VBUS values are present. | Supports [Tomi Linux USB Role Kernel Reference](../docs/reference/tomi-usb-role-kernel.md) and the Rake adjudication at `/mnt/d/Codex/usb-state-audit-the-rake-20260903/REPORT.md`. No explicit logged admission failure appears through the last retained dmesg dump; A versus B remains unresolved. Not evidence that admission succeeded or of behavior beyond coverage. |
...
| Evidence set | Type / location / identity | What it contains or establishes | Canonical use / limits |
|---|---|---|---|
| GPIO4 jumper removal and design closeout, 2026-09-24 | **Closeout / adjudication report** — `/mnt/d/Codex/TT3/20_GPIO4_JUMPER_REMOVAL_AND_DESIGN_CLOSEOUT_2026-09-24.md`; SHA-256 `22ebb2f564a19c524171886b37b970e2a7cc9c5f9a05f3097e8766777d17437b`. | Records final physical GPIO4 disposition and post-removal functional smoke results. | Supports [TomiDock Architecture](../docs/architecture/tomidock.md); pair with the adjacent lifecycle-investigation entry. Jumper removal closes current-topology GPIO4 necessity only, not deterministic detach fail-close or formal USB compliance. |
| GPIO4/BVALID lifecycle investigation, 2026-09-21 onward | **Instrumented evidence and adjudication set** — `/mnt/d/Codex/TT3/tomidock-gpio4-bvalid-audit-20260921/`; see `REPORT_PHASE4F_REDUCED_OBSERVER_DESIGN.md`, `REPORT_PHASE4G_ALTERNATING_REDUCED_OBSERVER_ADJUDICATION.md`, `phase4f_evidence/`, and `phase4g_evidence/`. The Phase 4F root, evidence, and firmware manifests and the Phase 4G evidence manifest were checked against their files. | Preserves reduced-observer design/build identity, capture packages and the alternating adjudication. Phase 4G produced four clean Phase 3 and four clean Phase 4F cycles without a meaningful observer-associated difference; A8 and two earlier premature-MOUNT observations remain unresolved. | Supports [TomiDock Architecture](../docs/architecture/tomidock.md); see the closeout entry above for final physical jumper status. Phase 4F is diagnostic evidence, not production firmware. Routine anomaly cycling was retired; deterministic no-GPIO physical-detach fail-close remains unproven. |

## TomiDock network ingress

| Evidence set | Type / location / identity | What it contains or establishes | Canonical use / limits |
|---|---|---|---|
| LAN-to-ECM inbound TCP boundary | **Bench finding / candidate report** — `/mnt/d/Codex/PPP-investigation/tomidock-boot-host-v001/REPORT.md`; the candidate itself is built/static-validated only, not deployed or bench-tested. | Routed LAN-to-ECM ICMP succeeded; direct unsolicited TCP/2121 to a proven Tomi listener timed out; explicit TCP/2323 mapping to Tomi port 23 worked; Tomi-originated TCP supported a successful active-FTP data path. | Supports the open ingress-policy boundary in [TomiDock Architecture](../docs/architecture/tomidock.md). Cause remains unresolved; do not generalize to all TCP or attribute the timeout to firewall, NAPT, lwIP, or firmware. |

## Power and support tooling

| Evidence set | Type / location / identity | What it contains or establishes | Canonical use / limits |
|---|---|---|---|
| PowerFlight v001.5.3 preserved run archive, 2026-09-21 | **Manifest-verified run archive** — `/mnt/d/Codex/TT3/Evidence/powerflight-20260921-170744-7150-00.tar.gz`; SHA-256 `67a8c41caa80c09ebe2fe9114bd01ba00b5ab84343b6ef750ccb58c5e52daf8a`. All six payload files matched the embedded `manifest.sha256`. | The archive records `VBUS=0`, `CHARGE=0`, `CHILD=1`, `USB_STATE_HOST`, `last_error=0x0`, and `BATTERY_RETURN_CHILD_ATTACHED`; its Tomi-specific kernel, USB-child, battery, GPIO/MMIO, and role records strongly support a physical Tomi bench run. | Supports [Tomi Power and Battery Reference](../docs/reference/tomi-power-battery.md) and [TomiDock Architecture](../docs/architecture/tomidock.md). The acquisition path and exact executed binary remain unbound; this is not universal electrical/charging proof or identification of the VBUS switch. |
