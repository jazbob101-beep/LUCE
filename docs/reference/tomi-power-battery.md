...
- **Formal validation:** a preserved package whose own file manifest verifies; it validates the captured software record, not unmeasured electrical quantities.
...
The v001.5.3 formal package is `/mnt/d/Codex/TT3/Evidence/powerflight-20260921-170744-7150-00.tar.gz`, SHA-256 `67a8c41caa80c09ebe2fe9114bd01ba00b5ab84343b6ef750ccb58c5e52daf8a`; its six payload entries verify against the embedded manifest. It records one validated run ending with:
...
This is coherent for the rebuilt rig: Tomi VBUS can be switched off while the USB data link remains attached and the TomiDock child stays enumerated. It establishes that the v001.5.3 classifier completed a battery-return observation with the recorded software signals in that run. It does not prove actual input or battery current, a universal hardware property, or a particular physical VBUS-switch implementation.

v001.5.3 materially corrected v001.5.2's conflation of “battery return” with physical USB cable removal. In the preserved contradiction run, the child remained present and role stayed HOST after VBUS/charge fell to zero; v001.5.2 waited 1535.410 seconds (about 25m35s) for detachment. v001.5.3 accepts either coherent PHYS-2 battery return (expected child + HOST while Tomi VBUS is off) or PHYS-1 return (no child + IDLE). The formal run reached completion about 5.08 seconds after WAIT_A_PRIME. These are observed run timings, not guarantees. The older defect was in the classifier/test topology model, not evidence of a Tomi charging defect.

The lineage is summarized in the reconciliation report. In brief: v001 established the observational A–B–C–B′–A′ recorder; v001.1 hardened qualification, baseline freezing, fail-visible states, and status persistence; v001.2 strengthened independent CHRG/status and topology invariants; v001.3 separated topology availability from core telemetry and fixed truthful readiness; v001.4 corrected exact `USB_STATE_*` authority strings; v001.5 adapted the procedure to the rebuilt topology; v001.5.1 restored fail-closed VBUS-loss handling; v001.5.2 hardened target runtime/documentation; v001.5.3 corrected battery-return classification. All remain software candidates unless individually identified otherwise; the formal archive is evidence for one v001.5.3 run.
